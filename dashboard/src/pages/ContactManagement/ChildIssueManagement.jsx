...
export default function ChildIssueManagement() {
  ...
  const fetchChildIssues = async () => {
    ...
    try {
      ...
      const { data } = await api.post("/api/admin/child-issue/all", body);
      if (data.success) {
        setChildIssues(data.services || []); // response key: services
      } else {
        toast.error(data.message || "Failed to fetch child issues");
      }
    } catch {
      toast.error("Server error while fetching child issues");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    const delay = setTimeout(fetchChildIssues, 500);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url =
        modal.type === "edit"
          ? `/api/admin/child-issue/${modal.data._id}`
          : "/api/admin/child-issue";
      const method = modal.type === "edit" ? api.patch : api.post;

      await method(url, formData);
      toast.success(
        `Child issue ${modal.type === "edit" ? "updated" : "created"} successfully`
      );
      setModal({ type: null, data: null });
      fetchChildIssues();
    } catch (err) {
      toast.error(err.response?.data?.message || "Error saving child issue");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this child issue?"))
      return;
    try {
      const { data } = await api.delete(`/api/admin/child-issue/${id}`);
      if (data.success) {
        toast.success("Child issue deleted successfully");
        fetchChildIssues();
      }
    } catch {
  ...
  const handleToggleStatus = async (issue) => {
    try {
      const { data } = await api.patch(`/api/admin/child-issue/${issue._id}`, {
        status: !issue.status,
      });
      if (data.success) {
        toast.success("Child issue status updated");
        fetchChildIssues();
      }
    } catch {
  ...
  const columns = useMemo(
    () => [
      {
        name: "Name",
        selector: (row) => row.name || row.issue,
        sortable: true,
      },
      {
        name: "Status",
      ...
      },
      {
        ...
        cell: (row) => (
          <RowActions
            ...
            onEdit={() => {
              setFormData({ name: row.name || row.issue, status: row.status });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center bg-gradient-to-r from-indigo-500 to-purple-500 p-5 rounded-xl shadow-md text-white">
        <h1 className="text-xl font-semibold">Child Issue Management</h1>
        {permissionLevel >= 2 && (
          <button
            onClick={() => {
          ...
          >
            <Plus size={18} /> Create Child Issue
          </button>
        )}
      </div>
        ...
        <Dialog.Panel className="bg-white w-full max-w-md rounded-xl shadow-xl p-6 space-y-4">
          <div className="flex justify-between items-center border-b pb-2">
            <Dialog.Title className="text-lg font-semibold text-gray-800">
              {modal.type === "view" && "View Child Issue"}
              {modal.type === "edit" && "Edit Child Issue"}
              {modal.type === "create" && "Create Child Issue"}
            </Dialog.Title>
            <button
          ...
          {modal.type === "view" ? (
            <ViewChildIssue childIssue={modal.data} />
          ) : (
...
/* --------------------------- VIEW CHILD ISSUE --------------------------- */
const ViewChildIssue = ({ childIssue }) => (
  <div className="space-y-3 text-gray-700">
    <p><strong>Name:</strong> {childIssue?.name || childIssue?.issue}</p>
    <p>
      ...
      >
        {childIssue?.status ? "Active" : "Inactive"}
      </span>
    </p>
    <p>
      <strong>Created:</strong>{" "}
      {new Date(childIssue?.createdAt).toLocaleString()}
    </p>
  </div>
);
